...
import edu.miu.cs489.tsogt.lab_9.model.*;
import edu.miu.cs489.tsogt.lab_9.repository.*;
...
public class DataLoader implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        var findAdmin = roleRepository.findByName("ROLE_ADMIN");
        var findUser = userRepository.findByUsername("admin");
        User newAdminUser = new User("Admin", "Admin", "Admin", "admin", passwordEncoder.encode("test1234"), "admin@gmail.com", true, true, true, true);

        if (findAdmin == null) {
            Role adminRole = new Role("ROLE_ADMIN");
            if (findUser == null) {
                newAdminUser.setRoles(Collections.singletonList(adminRole));
                userRepository.save(newAdminUser);
            }
        } else if (findUser == null) {
            newAdminUser.setRoles(Collections.singletonList(findAdmin.get()));
            userRepository.save(newAdminUser);
        }
    }
}
